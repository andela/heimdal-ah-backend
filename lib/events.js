...
const articleEvent = async (info) => {
  ...
  allReturnedFollowers.forEach(async (follower) => {
    const recipient = await UserModelQuery.getUserByNotification(follower.dataValues.followerId);
...
const commentEvent = async (info) => {
  ...
  userIds.forEach(async (notifyUserId) => {
    const recipient = await UserModelQuery.getUserByNotification(notifyUserId);
...
const likesEvents = async (payload, likes) => {
  ...
  const recipient = await UserModelQuery.getUserByNotification(articleOwner.id);
...
const followEvent = async (info) => {
  ...
  const recipient = await UserModelQuery.getUserByNotification(FollowedUser);
...
const ratingEvent = async (info) => {
  ...
  const recipient = await UserModelQuery.getUserByNotification(id);
